...
- 测试时间：2026-04-30 09:46:30
- 健康检查：✓ OK (1233ms) — {"status":"ok"}
- 用例总数：15
...
|---|---|---|---|---|---|
| **EN-01** | 英文短句，显式 language=en，自定义标签 | ✓ 200 | 5 | 5/5 | 1468ms |
| **EN-02** | 英文长段，labels 留空触发默认双语标签集 | ✓ 200 | 5 | 1/4 | 1593ms |
| **ZH-01** | 中文现代商业文本，显式 language=zh | ✓ 200 | 6 | 4/4 | 1224ms |
| **ZH-02** | 中文医疗场景，自定义双语标签 | ✓ 200 | 5 | 1/3 | 1252ms |
| **ZH-03** | 古典文学边界测试 — 「尤氏来请」应只取「尤氏」 | ✓ 200 | 6 | 6/6 | 1249ms |
| **AR-01** | 阿拉伯语新闻 | ✓ 200 | 4 | 2/2 | 1443ms |
| **MIX-01** | 中英混合 · 职场场景，language=mixed 强制双跑 | ✓ 200 | 7 | 4/7 | 1640ms |
| **MIX-02** | 学术场景，labels 留空 | ✓ 200 | 4 | 3/5 | 1666ms |
| **AUTO-01** | 纯中文文本，应被检测为 zh | ✓ 200 | 3 | 3/3 | 1223ms |
| **AUTO-02** | 纯英文文本，应被检测为 en | ✓ 200 | 4 | 3/3 | 1965ms |
| **AUTO-03** | 中英混合，应被检测为 mixed 并双跑合并 | ✓ 200 | 4 | 3/3 | 1472ms |
| **MIN-01** | min_entities=10 强制兜底（短文本启发式只期望 1 个） | ✓ 200 | 1 | 1/1 | 1464ms |
| **MIN-02** | min_entities=0 关闭兜底 | ✓ 200 | 1 | 1/1 | 1150ms |
| **THR-01** | 高阈值 0.8 - 期望返回更少但更高置信度的实体 | ✓ 200 | 1 | 1/3 | 1622ms |
| **EDGE-01** | 空文本 | ✓ 200 | 0 | — | 1192ms |

- 通过率：**15/15**
- 累计耗时：**21624ms**（平均 1442ms/请求）
...
**期望命中** 5/5：`California`, `2002`, `Hawthorne`, `Elon Musk`, `SpaceX`  
...
**期望命中** 1/4：`Washington D.C.`, `Chuck Schumer`, `Democratic Party`, `Biden`  
...
**期望命中** 2/2：`المملكة العربية السعودية`, `محمد بن سلمان`  
...
**期望命中** 4/7：`2024`, `张伟`, `北京`, `Sarah Chen`, `Meta`, `Google`, `Android`  
**未命中**：`2024`, `北京`, `Android`  
...
**期望命中** 3/5：`李明`, `清华大学`, `Transformer`, `MIT`, `Stanford University`  
**未命中**：`清华大学`, `Transformer`  
...
**期望命中** 3/3：`Tim Cook`, `Apple`, `Cupertino`  
...
**响应**：HTTP 200 · 1622ms · 1 个实体

| 文本 | 标签 | 置信度 | 起止 |
|---|---|---|---|
| `Elon Musk` | 人名或姓名 | 0.86 | 42–51 |

**期望命中** 1/3：`Elon Musk`, `SpaceX`, `Tesla`  
**未命中**：`SpaceX`, `Tesla`  
...
| 分组 | 用例数 | 最快 | 最慢 | 平均 |
|---|---|---|---|---|
| EN — GLiNER 主路径 | 2 | 1468ms | 1593ms | 1531ms |
| ZH — BERT 主路径 | 2 | 1224ms | 1252ms | 1238ms |
| ZH — BERT 边界识别 | 1 | 1249ms | 1249ms | 1249ms |
| AR — GLiNER 主路径 | 1 | 1443ms | 1443ms | 1443ms |
| Mixed — 双模型合并 | 2 | 1640ms | 1666ms | 1653ms |
| auto — 自动语言检测 | 3 | 1223ms | 1965ms | 1554ms |
| min_entities 覆盖启发式 | 2 | 1150ms | 1464ms | 1307ms |
| Threshold 变化 | 1 | 1622ms | 1622ms | 1622ms |
| Edge cases | 1 | 1192ms | 1192ms | 1192ms |
